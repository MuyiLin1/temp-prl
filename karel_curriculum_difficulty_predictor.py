...
This script implements the reward-conditioned task difficulty idea from
prl_task_generation.pdf for Karel task files such as clean_house.py.

What it does:
  1. Extracts cheap task descriptors phi(tau) from a Karel task source file:
     reward features, graph/topological features, spectral graph features,
     and programmatic/source-code features.
  2. If prior task measurements are supplied, fits a simple ridge predictor
     of policy/program performance and computes:
        RTD_n(tau) = sqrt(phi(tau)^T (lambda I + sum_i phi_i phi_i^T)^-1 phi(tau))
        predicted success probability
        total predictive difficulty
        curriculum utility
  3. Writes a JSON and CSV report.
...
Typical usage:
  python karel_curriculum_difficulty_predictor.py \
      ...
      --target-policy my_policy_or_current_agent \
      --out cleanhouse_difficulty_report.json

If you do not yet have prior measurements, run:
  python karel_curriculum_difficulty_predictor.py --task-file clean_house.py --write-prior-template prior_template.csv

Prior CSV format:
  Required columns:
    task_name, policy_id, return
  Optional columns:
    success
  Feature columns may be omitted. If omitted, the script cannot infer features for
  arbitrary prior tasks, so use --write-prior-template and fill/extend it, or use
  the helper output from this script to build rows task-by-task.

Notes:
  - This script is intentionally lightweight. It does not run the Karel simulator.
  - For exact policy-dependent predicted success, you need prior rows for the same
    policy_id / current agent. Without prior rows, the script computes descriptors
    only and sets model-based scores to null.
"""
...
def extract_world_map(source: str) -> Optional[List[List[Any]]]:
    """Extract the first literal assigned to a variable named world_map."""
...
def free_cells_from_world_map(world_map: List[List[Any]]) -> Tuple[List[Tuple[int, int]], set[Tuple[int, int]]]:
    free, walls = [], set()
...
def build_grid_graph(free_cells: Sequence[Tuple[int, int]]) -> Dict[Tuple[int, int], List[Tuple[int, int]]]:
    free_set = set(free_cells)
    graph: Dict[Tuple[int, int], List[Tuple[int, int]]] = {c: [] for c in free_cells}
...
def bfs_distances(graph: Dict[Tuple[int, int], List[Tuple[int, int]]], start: Tuple[int, int]) -> Dict[Tuple[int, int], int]:
...
def connected_components(graph: Dict[Tuple[int, int], List[Tuple[int, int]]]) -> List[List[Tuple[int, int]]]:
    seen = set()
    comps = []
    for node in graph:
        if node in seen:
...
def bridges_and_articulations(graph: Dict[Tuple[int, int], List[Tuple[int, int]]]) -> Tuple[int, int]:
    """Tarjan bridge/articulation counts for an undirected graph."""
    time = 0
...
def graph_laplacian_eigs(graph: Dict[Tuple[int, int], List[Tuple[int, int]]], k: int = 6) -> List[float]:
...
def estimate_cleanhouse_marker_count(source: str) -> float:
    """Best-effort static estimate from CleanHouse code: possible_marker_locations[:10] plus one near start."""
...
def infer_reward_features(source: str, task_name: str) -> Dict[str, float]:
    lower = (source + "\n" + task_name).lower()
    return {
        "reward_pick_markers": float("pickmarker" in lower or "picked-up" in lower or "pick up" in lower or "markers_grid.sum" in lower),
        ...
        "reward_num_subgoals_est": estimate_cleanhouse_marker_count(source) if "cleanhouse" in lower else 1.0,
        "reward_terminal_only": float("reward = 1." in lower and "num_markers == 0" in lower),
    }


def cleanhouse_descriptor(task_file: str | Path, task_name: str = "CleanHouse") -> Tuple[Dict[str, float], Dict[str, Any]]:
    ...
    # Exact all-pairs diameter is fine for CleanHouse-scale grids.
    diameter = 0
...
def load_prior_csv(path: str | Path) -> Any:
    if pd is None:
        raise RuntimeError("pandas is required to read prior CSV files. Install pandas or run without --prior-csv.")
...
def fit_and_score(
    ...
    # Feature columns are the intersection of candidate feature names and prior CSV columns.
    ...
    # Standardize using prior distribution. This makes RTD well-conditioned across feature scales.
    mu = X.mean(axis=0)
    sigma = X.std(axis=0)
    ...
    # Ridge regression on returns/success. Outputs are clipped to [0, 1].
    # Include intercept by augmenting X with ones; RTD still uses descriptor-only V above.
    X_aug = np.column_stack([np.ones(Xs.shape[0]), Xs])
    x_aug = np.concatenate([[1.0], xs])
    A = X_aug.T @ X_aug + lambda_ridge * np.eye(d + 1)
    A[0, 0] -= lambda_ridge  # do not regularize intercept
    coef = np.linalg.pinv(A) @ X_aug.T @ y
    pred_return = float(np.clip(x_aug @ coef, 0.0, 1.0))

    # If y was binary, pred_return is already success-ish; otherwise map return to success proxy.
...
def write_prior_template(path: str | Path, features: Dict[str, float], task_name: str, target_policy: str) -> None:
    fieldnames = ["task_name", "policy_id", "return", "success"] + list(features.keys())
    with Path(path).open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        row = {"task_name": task_name, "policy_id": target_policy, "return": "", "success": ""}
        row.update(features)
        writer.writerow(row)


def write_candidate_feature_csv(path: str | Path, features: Dict[str, float], task_name: str) -> None:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Compute PRL/Karel curriculum difficulty descriptors and RTD scores.")
    ...
    if args.write_prior_template:
        write_prior_template(args.write_prior_template, features, args.task_name, args.target_policy or "current_policy")
        print(f"Wrote prior-measurement template to {args.write_prior_template}")
        return

    if args.prior_csv:
        scores = fit_and_score(features, args.prior_csv, args.target_policy, args.lambda_ridge, args.alpha, args.beta)
    else:
        scores = ScoreResult(None, None, None, None, None, None, 0, 0, args.lambda_ridge, args.alpha, args.beta)
